    ...
    <div class="col-lg-12">
        <div class="panel">
                ...
                <div class="panel-body">
                    ...
                    if ($access == 3) { ?>
                        <div class="col-lg-12">
                            <a href="<?=base_url();?>referensi/satker/add" type="button" class="btn btn-primary"><i class="fa fa-plus"></i> <strong>TAMBAH DATA SATKER</strong></a>
                            <br><br><br>
                        </div>
                          ...
                          <table id="tableMDI" class="table table-striped table-bordered">
                            ...
                            <?php $no=1; foreach ($record as $row) { ?>
                              <tr>
                                ...
                                <td><?= $row->alamat ?></td>
                                <td><?= $row->telp ?><br><?= $row->email ?></td>
                                <td>
                                    <?php if ($access == 3 || $this->session->satker_kode == $row->kode_satker) { ?>
                                        <button onclick="location.href='<?=base_url('referensi/satker/edit/'.$row->kode_satker);?>'" type="button" class="btn btn-xs btn-primary"><i class="fa fa-pencil"></i>&nbsp;Ubah</button>
                                    <?php } ?>
                                        <button onclick="location.href='<?=base_url('referensi/satker/detail/'.$row->kode_satker);?>'" type="button" class="btn btn-xs btn-info"><i class="fa fa-search"></i>&nbsp;Detail</button>
                                </td>
                              </tr>
                            <?php $no++; } ?>
                            </tbody>
