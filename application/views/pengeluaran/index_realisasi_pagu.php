    ...
    <div class="col-lg-12">
        <div class="panel">
                <div class="panel-header">
                  <h3 class="panel-heading">Data Realisasi Pagu Anggaran Tahun <?= $tahun_exist ?> <?= $filter_tgl_exist ?> 
                    <?php 
                    $access = $this->session->sub_role;
                    if (!$access) { $access = 0; }
                    if ($access == 3) { ?>
                        
                            <!-- <a href="<?=base_url();?>pengeluaran/realisasihilang" type="button" class="btn btn-danger pull-right"><i class="fa fa-refresh"></i> <strong>Cek Realisasi Pagu Tidak Terdeteksi</strong></a> -->
                  ...
                  <div class="col-lg-12">
                    <form method="post" role="form" class="form-horizontal">
                        <div class="form-group">
                          <label class="col-lg-4 control-label">Filter Hari</label>
                          <div class="col-lg-3 inputGroupContainer date">
                            <div class="input-group input-append date">
                                <input type="text" class="form-control" id="inputTglIns0" name="filter_tgl" style="cursor: pointer" value="<?= date('Y-m-d') ?>" placeholder="YYYY-MM-DD"/>
                                <span class="input-group-addon add-on"><span class="glyphicon glyphicon-calendar"></span></span>
                            </div>
                          </div>
                          <div class="col-lg-3">
                            <button type="button" class="btn btn-warning" onClick="this.form.submit()">Filter</button>
                            <a href="<?php base_url('pengeluaran/realisasi') ?>" class="btn btn-success">Reset</a>
                          </div>
                        </div>
                    </form>
                  </div>
                            ...
                            <?php $no=1; foreach ($record as $row) { 
                              ...
                              <tr>
                                ...
                                <td>
                                  ...
                                  ?>
                                    <a href="<?=base_url();?>pengeluaran/addrealisasipagu/<?= $row->kdindex ?>"><?= $keterangan ?></a>
                                    
                                </td>
                                <td align="right"><?= number_format($row->pagu, 0, ',', '.'); ?></td>
                                <td align="right"><?= number_format($row->jml_realisasi_pagu, 0, ',', '.'); ?></td>
                                <td align="right"><?= number_format($row->sisa_pagu, 0, ',', '.'); ?></td>
                                <td align="right">
                                  <?php
                                    $target = $row->pagu;
                                    $real = $row->jml_realisasi_pagu;
