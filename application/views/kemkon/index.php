    ...
    <div class="col-lg-12">
        <div class="panel">
                <div class="panel-header">
                  ...
                  <div class="col-lg-12 pull-right" style="margin-bottom: 25px !important;">
                    <form  id="filterFormKemkon" method="post" role="form" class="form-horizontal">
                ...
                <div class="panel-body">
                    ...
                    if ($access == 2) { ?>
                        <div class="col-lg-12">
                            <a href="<?=base_url();?>pemberdayaan/kemkon/add" type="button" class="btn btn-primary"><i class="fa fa-plus"></i> <strong>TAMBAH DATA</strong></a>
                            <a id="btnUnduhKemkon" type="button" class="btn btn-warning"><i class="fa fa-download"></i> <strong>UNDUH DATA</strong></a>
                            <a href="<?=base_url();?>pemberdayaan/kemkon/resume" type="button" class="btn btn-success"><i class="fa fa-dashboard"></i> <strong>RESUME DATA</strong></a>
                            <br><br><br>
                        </div>
                    ...
                    <div class="table-responsive col-xs-12">
                          <table id="tableMDI" class="table table-striped table-bordered">
                            <thead>
                              <tr>
                                <th width="5%">NO</th>
                                <th width="25%">NAMA SATKER / UNIT KAWASAN</th>
                                <th width="25%">NAMA KELOMPOK / DESA</th>
                                <th width="35%">NO PKS / JUDUL</th>
                                <th width="10%">AKSI</th>
                              </tr>
                            </thead>
                            <tbody>
                            <?php $no=1; foreach ($record as $row) { ?>
                              <tr>
                                <td><?= $no ?></td>
                                <td><?= $row->nama_satker ?><br><?= $row->short_name.'. '.$row->nama_kk ?></td>
                                <td><?= $row->nama_kelompok ?><br><?= load_controller('Kemkon', 'getdatadesaview', $row->desa_id, NULL, 'pemberdayaan/') ?></td>
                                <td>Judul : <?= $row->judul_pks ?><br>No : <?= $row->no_pks ?></td>
                                <td>
                                    <?php if ($this->session->sub_role == 3 || $this->session->satker_kode == $row->satker_kode) { ?>
                                        <button style="margin-bottom: 5px !important;" onclick="location.href='<?=base_url('pemberdayaan/kemkon/edit/'.$row->id_kemitraan);?>'" type="button" class="btn btn-xs btn-primary"><i class="fa fa-pencil"></i>&nbsp;Ubah</button>
                                    <?php } ?>
                                        <button onclick="location.href='<?=base_url('pemberdayaan/kemkon/detail/'.$row->id_kemitraan);?>'" type="button" class="btn btn-xs btn-info"><i class="fa fa-search"></i>&nbsp;Detail</button>
                                </td>
                              </tr>
                            <?php $no++; } ?>
                            </tbody>
...
<script type="text/javascript">
  $('#btnUnduhKemkon').click(function(){ 
      $('#filterFormKemkon').attr('action','<?= base_url('pemberdayaan/kemkon/ekspor') ?>');
      var form = document.getElementById("filterFormKemkon");
      form.submit(); 
      $('#filterFormKemkon').attr('action','');
  });
</script>
<?php
