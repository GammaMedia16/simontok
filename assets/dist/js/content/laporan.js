$(document).ready(function () {
    ...
    .formValidation({
        ...
        fields: {
            filelaporan: {
                validators: {
                    notEmpty: {
                        message: 'File Laporan tidak boleh kosong.'
                    },
                    file: {
                        extension: 'pdf,jpg,png,doc,docx,xls,xlsx,ppt,pptx',
                        type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.openxmlformats-officedocument.presentationml.presentation,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/pdf,application/vnd.ms-excel,application/vnd.ms-powerpoint,application/msword,image/jpeg,image/png',
                        maxSize: 30*1024*1024,
                        message: 'File Foto harus berformat PDF,DOC,DOCX,XLS,XLSX,JPG,PNG,PPT,PPTX dengan ukuran maksimal 30 MB'
                    }
                }
            },
            judul_laporan: {
        ...
        var data = new FormData($("#createLaporanForm")[0]);
        $.ajax({
              type: 'POST',
              url: 'createlaporan',
              data: data,
              ...
              success: function(data) {
                ...
                else if (status == 'error')
                {
                    ...
                    $("#createLaporanForm").data('formValidation').resetForm();
                }
              }
          });
    });
    ...
    .formValidation({
        ...
        var $form     = $(e.target),
        validator = $form.data('formValidation');
        var data = new FormData($("#updateLaporanForm")[0]);
        $.ajax({
              type: 'POST',
              url: '../update',
              data: data,
              ...
              success: function(data) {
                ...
                else if (status == 'error')
                {
                    ...
                    $("#updateLaporanForm").data('formValidation').resetForm();
                }
              }
          });
    });
...
function deleteDataLaciFile(id) {
    var dialog = confirm("Yakin akan menghapus file ini?");
    if (dialog == true) {
    $.ajax({
          type: 'POST',
          url: 'hapuslaporan',
...
function detailLaporan(id){
    $.ajax({
          ...
          success: function(data) {
            ...
            $(".modal-title").html("Detail Laporan Kegiatan");
            ...
            $.each(row, function(i, r) {
                ...
                var titik2 = ":";
                if (i == "file_laporan") {
                    row[i] = "<a  target=\"_blank\" href=\"./assets/filelaporan/" + row[i] +"\">" + row[i] + "</a>";
                }
                if (i == "user_input") {
                    field = "Operator";
                    row[i] = row['nama_user'];
